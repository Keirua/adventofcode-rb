...
RSpec.describe Year2019::Day05 do
  ...
  it 'properly jump tests using position mode' do
    ic = IntCodeProgram.new([3, 12, 6, 12, 15, 1, 13, 14, 13, 4, 13, 99, -1, 0, 1, 9], 0)
    ic.run_program
    expect(ic.out).to eq([0])
    ic = IntCodeProgram.new([3, 12, 6, 12, 15, 1, 13, 14, 13, 4, 13, 99, -1, 0, 1, 9], 42)
    ic.run_program
    expect(ic.out).to eq([1])
  end

  it 'properly jump tests using position mode' do
    ic = IntCodeProgram.new([3, 3, 1105, -1, 9, 1101, 0, 0, 12, 4, 12, 99, 1], 0)
    ic.run_program
    expect(ic.out).to eq([0])
    ic = IntCodeProgram.new([3, 3, 1105, -1, 9, 1101, 0, 0, 12, 4, 12, 99, 1], 42)
    ic.run_program
    expect(ic.out).to eq([1])
  end
end
